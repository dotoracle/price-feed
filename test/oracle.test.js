...
describe("Bridge {}", async () => {
  ...
  it("submit read", async () => {
    ...
    for (var i = 0; i < signerAddresses.length; i++) {
      ...
      const PriceFeedTemp = await PriceFeedOracle.attach(priceFeedAddress)
      await PriceFeedTemp.connect(signers[i])
      await PriceFeedTemp.withdrawPayment(signerAddresses[i], signerAddresses[i], withdrawablePayment)
