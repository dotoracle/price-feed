...
module.exports = async (hre) => {
  ...
  log('  Deploying MultiPriceFeedOracleV2...');
  let dtoTokenAddress
  let oracles = []
  if (chainId == 31337) {
  ...
  const MultiPriceFeedOracleV2 = await ethers.getContractFactory('MultiPriceFeedOracleV2');
  const MultiPriceFeedOracleV2Instance = await MultiPriceFeedOracleV2.deploy()
  const multiPriceFeedOracleV2 = await MultiPriceFeedOracleV2Instance.deployed()
  await multiPriceFeedOracleV2.initialize(dtoTokenAddress, paymentAmount, deviationCheckerAddress, _description, tokenList)
  log('  - MultiPriceFeedOracleV2:         ', multiPriceFeedOracleV2.address);
  console.log('oracles', oracles)

  if (chainId != 31337) {
    const ERC20Mock = await ethers.getContractFactory('ERC20Mock')
    const tokenContract = await ERC20Mock.attach(dtoTokenAddress)
    await tokenContract.transfer(multiPriceFeedOracleV2.address, BigNumber.from(10).pow(18).mul(10000))
    await multiPriceFeedOracleV2.changeOracles(
      [],
      oracles,
      oracles,
      { gasLimit: 10000000 }
    )
    await multiPriceFeedOracleV2.changeOracleValidator(oraclesToAdd[0])
  }

  deployData['MultiPriceFeedOracleV2' + _description] = {
    abi: getContractAbi('MultiPriceFeedOracleV2'),
    address: multiPriceFeedOracleV2.address,
    deployTransaction: multiPriceFeedOracleV2.deployTransaction,
  }
...
module.exports.tags = ['multipricefeedv2']
